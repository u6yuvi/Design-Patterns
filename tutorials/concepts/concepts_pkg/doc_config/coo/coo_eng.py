...
__DOC_CONFIG_DIR = __PACKAGE_ROOT / "doc_config/coo/"


class COO_ENG_KEYWORD_CONFIG(BaseModel):
...
class COO_ENG_CONFIG(BaseModel):
    """Doc Level Configuration"""

    field: str
    field_config: COO_ENG_KEYWORD_CONFIG
...
def load_coo_config():
    """
    Load COO Document Configuration.
    """

    coo_config_json = load_document_config_json("coo", "coo_eng")


    bol_eng_config = [
        COO_ENG_CONFIG(
            field=key, field_config=COO_ENG_KEYWORD_CONFIG(**value)
        )
        for key, value in coo_config_json["fields_to_extract"].items()
    ]
    return bol_eng_config


if __name__ == "__main__":
    bol_config=load_coo_config()
